...
func (p *SlurmParser) BuildSqueueCommand(req models.SqueueRequest) string {
	cmd := "squeue"
...
func (p *SlurmParser) ParseSqueueOutput(output string, hasHeader bool) ([]models.QueueJobInfo, error) {
	...
	for i := startLine; i < len(lines); i++ {
		...
		job, err := p.parseSqueueJobLine(line)
		if err != nil {
			// 记录解析错误但继续处理其他行
...
func (p *SlurmParser) parseSqueueJobLine(line string) (models.QueueJobInfo, error) {
...
func (p *SlurmParser) ParseSqueueDetailedOutput(output string) ([]models.QueueJobInfo, error) {
	...
	for scanner.Scan() {
		...
		if strings.HasPrefix(line, "JobId=") {
			...
			p.parseSqueueDetailedJobLine(line, currentJob)
...
func (p *SlurmParser) parseSqueueDetailedJobLine(line string, job *models.QueueJobInfo) {
	// 解析 key=value 格式的字段
...
func (p *SlurmParser) ValidateSqueueRequest(req models.SqueueRequest) error {
	// 状态验证
