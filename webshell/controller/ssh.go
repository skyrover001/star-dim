...
func (t *Transfer) Write(p []byte) (n int, err error) {
	...
	if t.CastRecorder != nil {
		t.CastRecorder.Lock()
		t.CastRecorder.WriteData(OutPutType, string(p))
...
func NewTransfer(wsConn *websocket.Conn, sshClient *ssh.Client, rec *Recorder, cluster string, user string) (*Transfer, error) {
