...
// SinfoParser 封装 sinfo 命令的构建和解析逻辑
type SinfoParser struct{}

// NewSinfoParser 创建新的 SinfoParser 实例
func NewSinfoParser() *SinfoParser {
	return &SinfoParser{}
}

// BuildCommand 根据请求参数构建 sinfo 命令
func (p *SinfoParser) BuildCommand(req models.SinfoRequest) string {
	cmd := "sinfo"
...
func (p *SinfoParser) ParseOutput(output string, req models.SinfoRequest) (interface{}, error) {
...
func (p *SinfoParser) parseNodeOutput(output string, hasHeader bool, nodeCentric bool) ([]models.NodeInfo, error) {
	lines := strings.Split(strings.TrimSpace(output), "\n")
	if len(lines) == 0 {
		return []models.NodeInfo{}, nil
	}
...
func (p *SinfoParser) parseNodeLine(line string, nodeCentric bool) (models.NodeInfo, error) {
	// 使用空格分割字段
...
func (p *SinfoParser) parseSummaryOutput(output string, hasHeader bool) ([]models.NodeSummary, error) {
	lines := strings.Split(strings.TrimSpace(output), "\n")
	if len(lines) == 0 {
...
func (p *SinfoParser) parseSummaryLine(line string) (models.NodeSummary, error) {
	fields := strings.Fields(line)
	if len(fields) < 3 {
...
func (p *SinfoParser) parseReservationOutput(output string, hasHeader bool) ([]models.ReservationInfo, error) {
	lines := strings.Split(strings.TrimSpace(output), "\n")
	if len(lines) == 0 {
...
func (p *SinfoParser) parseReservationLine(line string) (models.ReservationInfo, error) {
	fields := strings.Fields(line)
	if len(fields) < 5 {
...
func (p *SinfoParser) ParseDetailedOutput(output string) ([]models.NodeInfo, error) {
...
func (p *SinfoParser) parseDetailedNodeLine(line string, node *models.NodeInfo) {
	// 解析 key=value 格式的字段
...
func (p *SinfoParser) parseMemory(memStr string) int {
...
func (p *SinfoParser) ValidateRequest(req models.SinfoRequest) error {
	// SSH 连接验证
	if req.Host == "" {
